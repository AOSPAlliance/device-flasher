	...
	window.SetContent(
		container.NewBorder(
			...
			container.NewGridWithColumns(2,
				...
				container.NewCenter(
					//&canvas.Image{Resource: , FillMode: canvas.ImageFillOriginal},
				),
			),
		),
	)
...
func preparation(logger *logrus.Logger, step int) {
	...
	case 2:
		...
		//screenshot = &canvas.Image{Resource: resourceWifisim, FillMode: canvas.ImageFillContain}
	case 3:
		...
		//screenshot = &canvas.Image{Resource: resourceBuildnumber, FillMode: canvas.ImageFillContain}
	case 4:
		...
		//screenshot = &canvas.Image{Resource: resourceOemunlocking, FillMode: canvas.ImageFillContain}
	case 5:
		...
		//screenshot = &canvas.Image{Resource: resourceEnableusbdebugging, FillMode: canvas.ImageFillContain}
	case 6:
		...
		//screenshot = &canvas.Image{Resource: resourceAllowusbdebugging, FillMode: canvas.ImageFillContain}
...
func (textGridWriter *scrollableTextGridWriter) Write(p []byte) (n int, err error) {
	...
	if strings.Contains(line, "Please") {
		...
		if strings.Contains(line, " lock ") {
			//screenshot = &canvas.Image{Resource: resourceBootloaderlocking, FillMode: canvas.ImageFillContain}
		} else {
			//screenshot = &canvas.Image{Resource: resourceBootloaderunlocking, FillMode: canvas.ImageFillContain}
		}
		screenshot.SetMinSize(fyne.Size{
			Width:  360,
			Height: 0,
		...
		bootloaderWarning := dialog.NewCustom("Hey, heads up...", "Done", container.NewBorder(nil, nil, nil, screenshot, widget.NewLabel(line)), window)
...
func success() {
	//screenshot := &canvas.Image{Resource: , FillMode: canvas.ImageFillContain}
	//screenshot.SetMinSize(fyne.Size{
	//	Width:  480,
	//	Height: 0,
	//})
		...
		container.NewBorder(
			...
			container.NewGridWithColumns(2,
				padding(5,
					container.NewVBox(
						layout.NewSpacer(),
						container.NewHBox(&canvas.Image{Resource: resourceSuccessSvg, FillMode: canvas.ImageFillOriginal}),
