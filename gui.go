...
func header(text string) *canvas.Text {
	return &canvas.Text{Color: fyne.CurrentApp().Settings().Theme().TextColor(), Text: text, TextSize: 36}
}

func body(text string) *fyne.Container {
	vbox := container.NewVBox()
	for _, line := range strings.Split(text,"\n") {
		vbox.Add(&canvas.Text{Color: fyne.CurrentApp().Settings().Theme().TextColor(), Text: line, TextSize: 16})
	}
	return vbox
}
			...
			container.NewGridWithColumns(2,
				padding(5,
					container.NewVBox(
						layout.NewSpacer(),
						header(title),
						body("Get ready for the most private"),
						body("mobile operating system in the world"),
						layout.NewSpacer(),
					),
				),
				splashScreen,
...
func preparation(logger *logrus.Logger, step int) {
	...
	screenshotContainer := container.NewMax(screenshot)
	window.SetContent(
		container.NewBorder(
			nil,
			container.NewHBox(
			...
			container.NewGridWithColumns(2,
				padding(5,
					container.NewVBox(
						...
						header("You've successfully installed " + Vendor + "!"),
...
func relock() {
	screenshot := container.NewMax(&canvas.Image{Resource: resources.ResourceOEMLockPng, FillMode: canvas.ImageFillContain})
	window.SetContent(
		container.NewBorder(
			nil,
			container.NewHBox(
